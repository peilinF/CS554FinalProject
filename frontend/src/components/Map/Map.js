...
const Map = () => {
  ...
  const logInfo = useSelector((state) => {
    if (state.logbook.logbook !== [] && state.logbook.selectedLog !== null) {
      ...
      return logbook.find((log) => log.uid === selectedLog);
    } else {
      return null;
    }
  });
  ...
  const friendIcons = useMemo(() => {
    return friends.map((user) => {
      let res =
        user.lastPosition.lat && user.lastPosition.lng && user.avatar
          ? {
              id: user.uid,

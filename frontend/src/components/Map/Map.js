...
const Map = () => {
  ...
  const findMe = () => {
    ...
    document.querySelector(".autocomplete input").placeholder =
      "Search location";
  ...
  const saveDirectionsToLogbook = async () => {
    try {
      const res = await apiInstance.post("/logbook/save-route", {
        userId: auth.currentUser.uid,
  ...
  const deleteRouteFromLogbook = async (routeId) => {
    ...
    try {
      const res = await apiInstance.get("/logbook/delete-route", {
        params: {
          userId: auth.currentUser.uid,
          routeId: routeId,
        },
  ...
  const createLog = async () => {
    ...
    try {
      const res = await apiInstance.get("/logbook/create-log", {
        params: {
          userId: auth.currentUser.uid,
          log_info: {
            date: date,
  ...
  const editLog = async () => {
    ...
    try {
      const res = await apiInstance.get("/logbook/edit-log", {
        params: {
          userId: auth.currentUser.uid,
  ...
  const deleteLogFromLogbook = async () => {
    ...
    try {
      const res = await apiInstance.get("/logbook/delete-log", {
        params: {
          userId: auth.currentUser.uid,
          logId: selectedLog._id,
        },
  ...
  let map_html = (
    ...
    >
      ...
      {showRoute && (
        <DirectionsRenderer
          directions={savedRoutes[routeIndex].ori_directions}
        />
      )}
      ...
      <ul className="saved-routes-ul">
        {savedRoutes &&
          savedRoutes.map((route, index) => (
            <div key={index}>
              <li className="saved-routes-li" key={index}>
                <p
                  onClick={() => {
                    setRouteIndex(index);
                    setShowRoute(true);
  ...
  let input_html = (
    <div>
      ...
      >
        <div>
          <label htmlFor="search-location">Start location:</label>
          <input id="search-location" type="text" placeholder="Enter Address" />
        </div>
      </Autocomplete>
      ...
      <h2>Generate random locations: </h2>
      ...
      <div>
        ...
        {!generater && (
          <Button variant={"contained"} onClick={() => generatePath()}>
            Generate
          </Button>
        )}
        {generater && (
      ...
      {selectedLog && (
        <div>
          ...
          <div className="logbook-edit-form">
            ...
            <button onClick={() => editLog()}>Submit</button>

...
const Directions = ({ input_distance, locations, handleDistance }) => {
      ...
      (result, status) => {
        if (status === google.maps.DirectionsStatus.OK) {
          ...
          if (status === google.maps.DirectionsStatus.ZERO_RESULTS) {
            alert(
              "No directions found. Try adjusting the starting point or radius, or try update random location."
            );
          }
        }
      }
  ...
  if (directions) {
    ...
    if (
      input_distance > 0 &&
      Math.abs(input_distance - distance) < input_distance / 10
    ) {
...
const Map = () => {
  ...
  const generateRandomLocations = () => {
    ...
    if (counter > 20) {
      alert(
        "No locations found. Try adjusting the starting point or radius, or try update random location."
      );
      setCounter(0);
      return;
    } else {
    ...
    for (let i = 0; i < numLocations; i++) {
      const lat =
        mapCenter.lat + (Math.random() * 2 - 1) * (radiusInKm / 111.32);
      const lng =
        mapCenter.lng +
        (Math.random() * 2 - 1) *
  ...
  let map_html = (
    ...
    >
      {/* <Directions /> */}
      {randomLocations.length > 0 && distance > 0 && (
        <Directions
          input_distance={distance}
          locations={[mapCenter, ...randomLocations, mapCenter]}
